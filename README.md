...
- 🗺️ **Roteirização Otimizada Avançada:** Geocodificação automática de endereços de clientes e CDs utilizando a biblioteca `geopy` (Nominatim). Cálculo da rota de entrega mais rápida utilizando dados reais de ruas e trânsito (OSRM) resolvidos via Google OR-Tools. Exibição interativa no app utilizando **OpenStreetMap** 100% gratuito (`flutter_map` e `latlong2`), com traçado de rota, pinos sequenciais de entrega e painel de navegação passo a passo dinâmico.
