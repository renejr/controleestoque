...
- 🗺️ **Roteirização Otimizada Mista:** Geocodificação resiliente com **enriquecimento automático de endereços via API do ViaCEP** antes da consulta ao `geopy` (Nominatim), garantindo precisão mesmo com dados sujos. Sequência ótima calculada com OR-Tools (VRP) para **Rotas Mistas (Entregas e Coletas na mesma viagem)**. As rotas urbanas são obtidas do **OSRM** com perfil terrestre (`/route/v1/driving/`) e geometria completa da via (`geometries=geojson&overview=full`). O Frontend renderiza a Polyline azul exatamente sobre as ruas, diferencia pinos de **CD** (galpão azul), **Entregas** (vermelho numerado) e **Coletas** (verde numerado), exibindo instruções dinâmicas no painel lateral (Desktop) ou BottomSheet (Mobile).
