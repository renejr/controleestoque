...
| **Módulo Fiscal e Identidade** | Configuração de dados fiscais (CNPJ, Regime), e Identidade Visual da empresa (Logo, Nome para PDFs) e Perfil de IA. | `routes/tenants.py`, `routes/tenant_settings.py`, `models/tenant.py`, `models/tenant_setting.py` | `screens/company_settings_screen.dart`, `widgets/product_form_modal.dart` | Tabela base e tabela de configurações isoladas e unificadas no Frontend. |
