...
| :--- | :--- | :--- | :--- | :--- |
| **Auth & Multi-Tenant** | Gerencia autenticação JWT, recuperação de senha (Mock) e isolamento de dados por locatário (Tenant). | `routes/auth.py`, `routes/tenants.py`, `models/tenant.py`, `models/user.py`, `core/security.py` | `services/auth_service.dart`, `providers/auth_provider.dart`, `screens/login_screen.dart`, `screens/mock_email_screen.dart`, `screens/reset_password_screen.dart` | Backend: Passlib, JWT, Bcrypt. Frontend: SharedPreferences (Lembrar-me). Todos os módulos dependem do Auth. |
...
| **Módulo Fiscal e Identidade** | Configuração de dados fiscais (CNPJ, Regime), e Identidade Visual da empresa (Logo na Tela de Login) e Perfil de IA. | `routes/tenants.py`, `routes/tenant_settings.py`, `models/tenant.py`, `models/tenant_setting.py` | `screens/company_settings_screen.dart`, `services/tenant_service.dart` | Tabela base e tabela de configurações isoladas e unificadas no Frontend. |
...
| **Oráculo de IA (LLM)** | IA Local atuando como CFO (Insights Financeiros em Markdown) e CSO (Plano de Reposição de Estoque em JSON e Chat Interativo). | `routes/dashboard.py`, `routes/oracle.py`, `services/llm_service.py` | `screens/ai_consultant_screen.dart`, `screens/oracle_restock_screen.dart`, `services/oracle_service.dart` | Backend: httpx, Ollama (Llama 3.2 1B). Frontend: flutter_markdown. |
...
| **TMS (Gestão de Frota e Logística)** | Cadastro de veículos, motor de Bin Packing 3D para calcular cubagem de carga e Roteirização otimizada a partir de Centros de Distribuição (CDs). Suporta Rotas Mistas (Entregas e Coletas combinadas). | `routes/vehicles.py`, `routes/fleet.py`, `models/vehicle.py`, `models/distribution_center.py` | `screens/vehicles_screen.dart`, `screens/distribution_center_management_screen.dart`, `screens/cargo_simulator_screen.dart`, `screens/route_viewer_screen.dart` | Backend: `py3dbp` (Bin Packing 3D), `ortools`, `geopy`, `httpx` (ViaCEP/OSRM), `reportlab` (PDF). Frontend: `flutter_map`, `latlong2`. |
| **Módulo de Coleta (Inbound)** | Gestão de Ordens de Coleta (Logística Reversa). Escalamento com vínculo a CDs e Veículos. Roteirização em lote e baixa no campo pelo motorista. | `routes/collections.py`, `models/collection_order.py`, `schemas/collection_order.py` | `services/collection_service.dart`, `screens/collection_list_screen.dart`, `screens/collection_form_screen.dart`, `screens/collection_route_screen.dart`, `screens/collection_execution_screen.dart` | Backend: Reaproveita motor OSRM/OR-Tools (`routing_service.py`). Frontend: Checkboxes dinâmicos, UI Adaptativa. |
...
*   **Infraestrutura Core:** Servidor FastAPI configurado, banco PostgreSQL rodando com Alembic para migrações. App Flutter consumindo API com tratamento de erros. Cloudflare Tunnels implementado para exposição rápida do App e API na internet.
*   **Segurança:** Multi-tenancy implementado via UUID. Autenticação JWT funcional (Login e persistência de sessão). Suporte robusto a SQLite FFI no Flutter para Web/Desktop com persistência via IndexedDB.
...
*   **Módulo de Coleta (Inbound/Logística Reversa):** Pipeline completo criado (`CollectionOrder`). Backend com modelo relacional atualizado (`street, number, city, distribution_center_id, vehicle_id`). Frontend possui tela de listagem adaptativa (com checkboxes nativos), tela de criação com combo-boxes vinculados ao TMS e painel Mobile-First para confirmação e baixa pelo motorista (preparado para câmera/scanner). Roteirização integrada ao OSRM.
*   **Roteirização Logística Avançada (Rotas Mistas):** Motor backend integrando `ViaCEP` (Enriquecimento pré-geocode), `Geopy` (Geocoding com cache), `OSRM` (Matrizes de Distância/Duração em malha viária + geometria de rota) e `OR-Tools` (VRP) para calcular a sequência ótima. O sistema unifica **Entregas (Outbound)** e **Coletas (Inbound)** no mesmo traçado. A API retorna a sequência, a geometria e os `steps` em português. O Frontend desenha a Polyline azul sobre as ruas, exibindo painéis laterais adaptativos com pinos diferenciados para CD (galpão azul), entregas (vermelho) e coletas (verde).
...
*   **Chat Corporativo (WebSockets):** Implementação de túnel em tempo real com isolamento de Tenant (`ConnectionManager`). Proteção contra bloqueios 403 (Handshake refatorado) e mecanismo de Heartbeat (Ping/Pong) para manter a conexão ativa contra ociosidade de rede. Frontend reativo com escuta global de mensagens (Badge) e UI de balões de mensagens.
...
*   **Módulo Fiscal e Identidade Visual:** Tela de configurações consolidada do Tenant (Razão Social, CNPJ, Regime) unida à gestão de identidade (`tenant_settings`), controlando a injeção da logo nos PDFs, na Tela de Login Pública e o tom de voz dinâmico (Conservador, Neutro, Agressivo) da IA no sistema. Banco de dados estruturado para a futura tabela de NF-e (`NfeDocument`).
...
*   **Inteligência Artificial (CFO e CSO):** Integração via Ollama consolidada. O LLM atua como CFO gerando análises formatadas em Markdown e como CSO sugerindo planos de reposição de estoque (JSON) e respondendo a dúvidas operacionais (Chat Interativo Responsivo).
