...
| **TMS (Gestão de Frota e Logística)** | Cadastro de veículos com capacidades dimensionais e de peso. Motor de Bin Packing 3D para calcular cubagem de carga e Roteirização OSRM + OR-Tools para despachos otimizados. | `routes/vehicles.py`, `routes/fleet.py`, `models/vehicle.py`, `services/logistics_service.py`, `services/routing_service.py`, `services/pdf_service.py` | `services/vehicle_service.dart`, `models/vehicle.dart`, `screens/vehicles_screen.dart`, `screens/cargo_simulator_screen.dart`, `screens/route_viewer_screen.dart`, `screens/widgets/truck_bed_visualizer.dart` | Backend: `py3dbp` (Bin Packing 3D), `ortools`, `geopy`, `reportlab` (PDF). Frontend: `flutter_map`, `latlong2`. |
...
*   **Roteirização Logística Avançada:** Motor backend integrando `Geopy` (Geocoding com cache), `OSRM` (Matrizes de Distância/Duração reais em malha viária) e `OR-Tools` (VRPTW) para calcular a rota mais rápida. Frontend exibe os pontos otimizados usando **OpenStreetMap** (via `flutter_map` e `latlong2`), sem dependência de chaves de API proprietárias, com suporte a redirecionamento para Waze/Google Maps.
