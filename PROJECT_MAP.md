...
| **TMS (Gestão de Frota e Logística)** | Cadastro de veículos com capacidades dimensionais e de peso. Motor de Bin Packing 3D para calcular cubagem de carga e Roteirização OSRM + OR-Tools para despachos otimizados. | `routes/vehicles.py`, `routes/fleet.py`, `models/vehicle.py`, `services/logistics_service.py`, `services/routing_service.py` | `services/vehicle_service.dart`, `models/vehicle.dart`, `screens/vehicles_screen.dart`, `screens/cargo_simulator_screen.dart`, `screens/route_viewer_screen.dart` | Backend: `py3dbp` (Bin Packing 3D), `ortools`, `geopy`. Frontend: `flutter_map`, `latlong2`. |
