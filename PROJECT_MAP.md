...
*   **Roteirização Logística Avançada:** Motor backend integrando `Geopy` (Geocoding com cache), `OSRM` (Matrizes de Distância/Duração em malha viária + geometria de rota via `geometries=geojson&overview=full` em `/route/v1/driving/`) e `OR-Tools` (VRP) para calcular a sequência ótima. A API retorna a sequência e a geometria da rota, além de `steps` em português compostos a partir de `type/modifier/name` (ex.: “Vire à direita na …”, “Na rotatória…”). O Frontend desenha a Polyline azul exatamente sobre as ruas (decodificando a geometria GeoJSON) e exibe instruções no painel: no Desktop, à direita; no Mobile, em BottomSheet; pinos diferenciados para CD (galpão azul), entregas numeradas em vermelho e coletas em verde.
