...
*   **Gestão de Fornecedores:** Módulo de Compras (Fase 1) implementado com CRUD de fornecedores (Validação de e-mail e layout responsivo).
...
1.  **Módulo Fiscal (Integração SEFAZ):**
    *   Adicionar campos fiscais no modelo `Product` (NCM, CFOP, CEST, Origem).
    *   Expandir o modelo `Tenant` para armazenar CNPJ, Razão Social e Regime Tributário.
    *   Preparar arquitetura para emissão de NFe baseada nas transações de `OUT`.
2.  **Módulo de Compras (Gestão de Fornecedores):**
    *   Criar modelos de Fornecedores e Ordens de Compra.
    *   Automatizar gatilhos: Ao aprovar Ordem de Compra, gerar transação de `IN` e atualizar contas a pagar.
3.  **Auditoria e Logs de Sistema:**
    *   Implementar middleware de tracking no FastAPI para registrar quem alterou o que (ex: alteração de preço de custo).
4.  **Aprimoramento de UI/UX Mobile:**
